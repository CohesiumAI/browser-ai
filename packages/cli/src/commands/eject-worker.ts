 ...
 * CDC v2026.8 §16.4 — DX External worker
 * 
  ...
  webllm: [
    {
      ...
      content: `/**
 * WebLLM Worker — External worker for CSP compliance.
 * Generated by @browser-ai/cli eject-worker
 * CDC v2026.8 §16.4
 */
  ...
  wasm: [
    {
      name: 'wasm-worker.js',
      content: `/**
 * WASM Worker — External worker for Transformers.js WASM inference.
 * Generated by @browser-ai/cli eject-worker
 * CDC v2026.8 §16.4
 */
...
export async function ejectWorker(options: EjectWorkerOptions): Promise<void> {
  ...
  for (const p of providers) {
    ...
    if (!files) {
      console.log(chalk.yellow(`  ⚠ Unknown provider: ${p}`));
      continue;
    }
    ...
    for (const file of files) {
      ...
      if (exists && !force) {
        console.log(chalk.yellow(`  ⏭ Skipped (exists): ${file.name}`));
        continue;
      }
...
`;
  ...
  if (!(await fs.pathExists(configPath)) || force) {
    ...
    console.log(chalk.green(`  ✓ Created: browser-ai.config.js`));
    filesWritten++;
  }

  // Summary
  console.log('\\n' + chalk.blue('━'.repeat(50)));
  console.log(chalk.green(`\\n✅ Ejected ${filesWritten} file(s) to ${output}\\n`));
